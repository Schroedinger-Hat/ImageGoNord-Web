...
MAX_POST_TO_PUBLISH = 5
REPOST_SUBREDDIT = ['wallpapers', 'wallpaper', 'minimalwallpaper']
REPOST_FREQUENCY = 4

if (REDDIT_USERNAME is None
        or REDDIT_PASSWORD is None
        or REDDIT_APP_ID is None
        or REDDIT_APP_SECRET is None):
...
def get_image_from_subreddit(subreddit):
    ...
    for submission in subreddit.new(limit=None):
        if (submission.url.endswith('jpg')
                ...
                or submission.url.endswith('bmp')):
...
print('Processable and unique wallpaper: ' + str(len(wallpapers_to_process)))
...
if len(wallpapers_to_process) > 0:
    ...
    base64image = base64.b64encode(requests.get(sub['url']).content)
    im = go_nord.base64_to_image(base64image)
